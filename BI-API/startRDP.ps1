...
try
{
    ...
    else
    {
        ...
        $requestId = Invoke-RestMethod -Uri "${baseUrl}Requests" -Method POST -Headers $headers -WebSession $session -Body @{AccessType="rdp"; SystemId=$ma.SystemId; AccountId=$ma.AccountId; DurationMinutes=$requestDurationInMinutes;};
        if ($verbose) { "..Request complete: $requestId"; ""; }

        #Create Session
        if ($verbose) {	"Creating RDP Session.."; }
        $sessionValue = Invoke-RestMethod -Uri "${baseUrl}Requests/${requestId}/Sessions" -Method POST -Headers $headers -WebSession $session -Body @{SessionType="rdpfile";};
