...
.Example
	Workflow:
	01. Copy all contents of CMD_PS_DIR_CERT.txt from MER capture
	02. Paste in $source location (in this example, we use C:\My\sample.txt)
	03. Make sure your $source variable is pointing to where you dropped the contents of CMD_PS_DIR_CERT.txt 
	04. Open PowerShell and run: .\CertCheck
	
	To use this script in order to check current system's missing certificates:
	01. Uncomment out the line "Dir cert: -recurse >> C:\My\sample.txt" and make sure the directory structure exists
...
<# Uncomment out the following line order (starting with "Dir cert") in order to a.) gather contents of current root certificates present on the system, and b.) check for any missing root certificates: #>
